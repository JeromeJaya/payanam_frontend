...
export default function VendorDashboard() {
  ...
  const fetchVendorBookings = async (page = 1) => {
    ...
    try {
      const response = await api.get(`/api/v1/bookings/vendor-bookings?page=${page}&limit=20`);
      if (response.data.success) {
        if (page === 1) {
          setVendorBookings(response.data.data);
        } else {
          setVendorBookings(prev => [...prev, ...response.data.data]);
        }
        setVendorBookingsTotal(response.data.pagination?.totalCount || 0);
  ...
  useEffect(() => {
    if (pendingFlightId) {
      setFlightRouteFormData(prev => ({ ...prev, flightId: pendingFlightId }));
    }
  }, [pendingFlightId]);
  ...
  const handleCreateSuccess = (newBus) => {
    ...
    setShowCreateBusForm(false);
    setPendingBusId(newBus._id);
    setRouteCategory("bus");
    setActiveTab("routes");
  ...
  const handleCreateFlightSuccess = (newFlight) => {
    ...
    setShowCreateFlightForm(false);
    setPendingFlightId(newFlight._id);
    setRouteCategory("flight");
    setActiveTab("routes");
  ...
  const handleFlightRouteSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        const createdRoute = response.data.data;
        ...
        setScheduleCategory("flight");
        setFlightScheduleFormData(prev => ({
          ...prev,
            ...
            {activeTab === "bookings" && (
              <div className="space-y-6">
                ...
                {vendorBookings.length > 0 && vendorBookings.length < vendorBookingsTotal && (
                  <button
                    onClick={() => fetchVendorBookings(vendorBookingsPage + 1)}
      ...
      {showCreateRouteForm && (
        <CreateRouteForm
          ...
          onSuccess={(createdRoute) => {
            ...
            setScheduleCategory("bus");
            setActiveTab("schedule");
            setScheduleFormData(prev => ({
              ...prev,
              busId: createdRoute.busId,
              routeId: createdRoute._id
            }));
          }}
        />
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-mono font-bold text-slate-900">{selectedBooking.bookingId}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-slate-900">₹{selectedBooking.totalFare?.toLocaleString('en-IN')}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-wider">Booked On</p>
                  <p className="text-sm text-slate-700">
                    {selectedBooking.bookedAt ? new Date(selectedBooking.bookedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'N/A'}
                  </p>
                </div>
              </div>
              ...
              <div className="border-t border-slate-200 pt-4">
                <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                  ...
                  Passenger Details ({selectedBooking.passengerDetails?.length || 0})
                </h3>
              ...
              <div className="border-t border-slate-200 pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {(selectedBooking.bookedSeats || []).map((seat, index) => (
                    <span key={index} className="px-3 py-1 bg-lime-100 text-lime-700 rounded-lg text-sm font-bold">
                      {seat}
                    </span>
                  ))}
                </div>
              ...
              <div className="border-t border-slate-200 pt-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-bold text-slate-900">{selectedBooking.boardingPoint?.city || 'N/A'}</p>
                    <p className="text-sm text-slate-600">{selectedBooking.boardingPoint?.name || ''}</p>
                    {selectedBooking.boardingPoint?.time && (
                      <p className="text-xs text-slate-500">Time: {selectedBooking.boardingPoint.time}</p>
                    )}
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 uppercase tracking-wider">Dropping Point</p>
                    <p className="font-bold text-slate-900">{selectedBooking.droppingPoint?.city || 'N/A'}</p>
                    <p className="text-sm text-slate-600">{selectedBooking.droppingPoint?.name || ''}</p>
                    {selectedBooking.droppingPoint?.time && (
                      <p className="text-xs text-slate-500">Time: {selectedBooking.droppingPoint.time}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {selectedBooking.userId && (
                <div className="border-t border-slate-200 pt-4">
                  ...
                  <div className="bg-slate-50 rounded-lg p-4">
                    <p className="font-bold text-slate-900">{selectedBooking.userId.name}</p>
                    <p className="text-sm text-slate-600">{selectedBooking.userId.email}</p>
                    {selectedBooking.userId.phoneNo && (
                      <p className="text-sm text-slate-600">{selectedBooking.userId.phoneNo}</p>
                    )}
                  </div>
                </div>
              )}
