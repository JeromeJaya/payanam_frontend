...
export default function OTPVerification() {
    ...
    const isMaxAttemptsExceeded = attempts >= MAX_ATTEMPTS;
    ...
    async function handleVerifyOTP(event) {
      ...
      if (attempts >= MAX_ATTEMPTS) {
        setError(`Maximum ${MAX_ATTEMPTS} attempts exceeded. Please request a new OTP.`);
        return;
      }
      ...
      >
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 sm:p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm">Enter the OTP sent to your email</p>
          </div>
          ...
          <form className="space-y-4 sm:space-y-5" onSubmit={handleVerifyOTP}>
            ...
            <div>
              ...
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && !loading && !isMaxAttemptsExceeded && handleVerifyOTP(e)}
                required
                placeholder="Enter 6-digit OTP"
                disabled={isMaxAttemptsExceeded}
                className={`w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-lime-500 dark:focus:ring-lime-400 focus:border-lime-500 dark:focus:border-lime-400 outline-none transition-colors text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 ${isMaxAttemptsExceeded ? "opacity-50 cursor-not-allowed" : ""}`}
              />
              ...
              {isMaxAttemptsExceeded && (
                <p className="text-red-500 text-xs mt-1">Too many failed attempts. Please request a new OTP.</p>
              )}
            </div>
            ...
            {loading ? (
              ...
              <But3
                ...
                text="Verify OTP"
                disabled={isMaxAttemptsExceeded}
                className={`w-full sm:w-fit block mx-auto bg-lime-600 dark:bg-lime-300 text-white dark:text-gray-900 py-3 px-4 rounded-lg font-medium hover:bg-lime-700 dark:hover:bg-lime-400 justify-center transition-colors ${isMaxAttemptsExceeded ? "opacity-50 cursor-not-allowed" : ""}`}
              />
            )}
            </div>
          </form>
